...
import React, { useEffect, useState } from "react";
import { jwtDecode } from "jwt-decode";
import { getWishlist, removeFromWishlist } from "../service/serviceWishlist";
import MainNavbar from "../navbar/MainNavbar";
import { FaTrashAlt } from "react-icons/fa";
...
function WishList() {
  ...
  const addToCart = (product) => {
    ...
    try {
      removeFromWishlist(userWishlist.id, product.productId);
      setWishlistProduct((prevProducts) =>
        prevProducts.filter((item) => item.productId !== product.productId)
      );
      console.log("Product removed from wishlist:", product.productId);
    } catch (err) {
      console.error("Failed to remove product from wishlist:", err);
    }
  };
